...
export const autoApproveBookings = onSchedule(
  ...
  async () => {
    ...
    for (const doc of snap.docs) {
      ...
      if (touristId) {
        notifications.push(() =>
          notify(touristId, "booking_auto_approved", undefined, `${doc.id}_auto_approved`)
        );
      }
      if (tutorId) {
        notifications.push(() =>
          notify(tutorId, "booking_guide_auto_approved", undefined, `${doc.id}_guide_auto_approved`)
        );
      }
    }
